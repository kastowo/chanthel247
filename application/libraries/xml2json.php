...
class xml2json {
  ...
  */    
  public static function convertSimpleXmlElementObjectIntoArray($simpleXmlElementObject, &$recursionDepth=0) { 
    //print_r($simpleXmlElementObject);exit();
    global $modx;
      ...
      $copyOfsimpleXmlElementObject = $simpleXmlElementObject;
       ...
       if (is_array($simpleXmlElementObject)) {
         ...
         if (count($simpleXmlElementObject) <= 0) {
           ...
           return (strval($copyOfsimpleXmlElementObject));
         }
       ...
       //if(self::$DEBUG) $modx->log(modX::LOG_LEVEL_ERROR, 'xml2json: foreach object: "'.json_encode($simpleXmlElementObject).'".');           

        if((is_string($key)) && ($key == self::$SIMPLE_XML_ELEMENT_OBJECT_PROPERTY_FOR_ATTRIBUTES)) {
    if(self::$DEBUG) $modx->log(modX::LOG_LEVEL_ERROR,'xml2json: Got attribute "'.$value.'".');    
    if(!self::$INCLUDE_ATTRIBUTES){
      ...
      $resultArray[$callerProvidedSimpleXmlElementObject->getName()] = $tempArray;
         ...
         return (strval($simpleXmlElementObject));
       } // End of else
